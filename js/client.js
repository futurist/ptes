...
function getArrayPath (data, path) {
  ...
  for (var i = 0; i < path.length; i++) {
    obj = type$1.call(obj) === ARRAY$1 ? obj[path[i]] : obj && obj.children && obj.children[path[i]];
    texts.push(obj.name);
  }
...
function convertSimpleData (d, prop, path) {
  ...
  if (type$1.call(d) === ARRAY$1) {
    return d
    // return d.map(function (v, i) {
    //   return convertSimpleData(v, prop, path.concat(i))
    // })
  }
  if (type$1.call(d) === OBJECT$1) {
    var node = [];
    for (var k in d) {
      if (k === '' && type$1.call(d[k]) === ARRAY$1) {
        node.push.apply(node, d[k].map(function (v, i) {
          return type$1.call(v) === OBJECT$1 ? v : Object.assign({name: v, _leaf: true}, prop && prop(v, path.concat(['', i])))
        }));
      } else {
...
var INVALID_NAME$1 = '<>:"\\|?*\/'; // '<>:"/\\|?*'
var INVALID_NAME_REGEXP$1 = new RegExp('[' + INVALID_NAME$1.replace('\\', '\\\\') + ']', 'g');
// disable right click
...
function getLeaf(node, f) {
  ...
  else { return treeHelper.deepFindKV(node.children, function (v){ return v._leaf==true; }) }
}
...
var com = {
  ...
  controller: function (args) {
    ...
    var undoManager$$1 = new undoManager();
    var undoRedo = function(redo, undo) {
      undoManager$$1.add({redo: redo, undo: undo});
      return redo
    };
    var redoList = [];
    ...
    function _extend (dest, src) {
      Object.keys(src)
        .filter(function (k) { return k[0] !== '_' && ['text', 'children'].indexOf(k) < 0; })
        .forEach(function (k) {
    ...
    function isValidTree(arg) {
      if( treeHelper.deepFindKV(data, function (v){ return v._edit; }, 1).length ){
    ...
    function oneAction (obj) {
      return m('a[href=#]', {class: 'action', onmousedown: function (e) {
        e.stopPropagation();
        e.preventDefault();
    ...
    function getAction (v) {
      ...
      if (!v._leaf) {
        node.push(
          ...
          {action: 'test', text: 'Test', _path:v._path, path: path, file:getLeaf(v).map(function (x){ return x.item.name; }), folder: folder, retain:true, url:url}
        );
      } else {
        node.push(
    ...
    function getInput (v) {
      if (v._leaf) {
        return [
          v.name ? m('div', v.name) : [],
          m('textarea', {
            config: function (el) { return el.focus(); },
            oninput: function (e) {
              v.desc = this.value;
        ...
        return Object.keys(v)
          .filter(function (k) { return k[0] !== '_' && k !== 'children'; })
          .map(function (k) { return m('.editline', [
            m('span', k),
            m('input', {
              // config: el => { el.focus() },
              value: v[k] || '',
          ...
          return {
            ...
            attrs: _extend({
              ...
              ondblclick: function (e) {
                ...
                Object.keys(v)
                  .filter(function (k) { return k[0] !== '_' && k !== 'children'; })
                  .forEach(function (k) { return oldVal[k] = v[k]; });
                undoRedo(
                  function() {
                  },
                  function () {
    ...
    function getMenu (items) {
      return items.map(function (v){ return m('a.button[href=#]',
               {
                 onclick: function (e) {
                   e.preventDefault();
                   v.action();
    ...
    ctrl.getDom = function (_) {
      return [
        m('.menu', [
          getMenu([
            {text:'Save', action:function (){saveConfig();} },
    ...
    function keyMoveLevel (e, key) {
      ...
      if (sel) {
        ...
        if (/right/.test(key) && child && child.length) {
          ...
          var oldNode$1 = sel.node;
          var pos = oldNode$1._pos || 0;
          sel.node = child[pos];
          sel.node._path = oldNode$1._path.concat(pos);
          sel.idx = pos;
          sel.parent = oldNode$1;
          if (oldNode$1._close) { oldNode$1._close = false; }
          m.redraw();
        }
      }
    }
    function keyMoveSibling (e, key) {
      ...
      var moveSibling = function (isMove) {
        if (isMove) { var assign;
          (assign = [child[sel.idx], child[newIdx]], child[newIdx] = assign[0], child[sel.idx] = assign[1]); }
    ...
    function doMoveCopy (e) {
      ...
      if (target.type) {
        ...
        var g = undoManager$$1.group(2);
        window.undo = undoManager$$1;
      }
      m.redraw();
    }
...
// type check helpers
var type$2 = {}.toString;
var ARRAY$2 = type$2.call([]);
var OBJECT$2 = type$2.call({});

// only array, object now treated as iterable
function isIterable (v) {
  return type$2.call(v) == OBJECT$2 || type$2.call(v) == ARRAY$2
}
...
function parseObj (d, result, node, init) {
  ...
  if (type$2.call(d) == OBJECT$2) {
    ...
    for (var k in d) {
      ...
      if (!isIterable(d[k]) || type$2.call(d[k]) == ARRAY$2 && !isIterable(d[k][0])) {
...
var m$2 = _interopDefault$1(mithril);

var type$3 = {}.toString;

function isObject(object) {
	return type$3.call(object) === "[object Object]"
}

function isString(object) {
	return type$3.call(object) === "[object String]"
}
...
var result = cssobj_cjs$1(style, {local:true});

var m$1 = cssobjMithril_cjs$1(result);
...
var footer = {
  ...
  view: function (ctrl, arg) {
    return m$1('.footerContent',{class:ctrl.getClass()},
              [format_1(
               ...
               arg.result ? m$1('a.button[href=#]',{onclick:function (e){ return arg.onclose&&arg.onclose(); }}, 'close')  : [],
              ])
  }
};
...
var testItem = {
  view: function(ctrl, arg) {
    return m$1('.testItem', [
      m$1('span', arg.test.msg),
      m$1('span', arg.test.submsg||''),
      m$1('span', arg.test.status||'?'),
      arg.test.error? m$1('a[href=#]', {onclick:function() {
...
var reporter = {
  ...
  view: function (ctrl, arg) {
    return m$1('.runner-result', [
      ctrl.result ? m$1('menu.top', [ m$1('a[href=#]',{onclick:function (e){ return arg.onclose&&arg.onclose(); }}, 'close') ]) : [],
      m$1('h3', {style: {margin: '1em 0 0 1em'}}, 'Result for ptest-runner'),
      m$1('.reporter',
         ctrl.data.map( function (v,i) {
           return m$1(
             '.item',
             {
             ...
             v.test
               ? m$1(testItem, Object.assign({},arg, {test:v}))
             : m$1('strong', v.msg + (v.result? ' '+v.result:''))
           )
         })
        ),
      m$1('.footer', m$1(footer, Object.assign({},arg,{
        result: ctrl.data[0].result,
        total: ctrl.data.filter(function (v) { return v.test; }),
        success: ctrl.data.filter(function (v) { return v.status == 'success'; }),
        fail: ctrl.data.filter(function (v) { return v.status == 'fail'; }),
      })))
    ])
  }
};
...
var result$1 = cssobj_cjs$1(style$1, {local:true});
var m$3 = cssobjMithril_cjs$1(result$1);
...
var gallary = {
  controller: function (arg) {
    ...
    var data = arg.data || testdata$1;
    var folder = data.folder;
    var test = data.test;
    var a = data.a;
    var images = m$3.request({method: 'GET', url: [PTEST_PATH, 'testimage'].join(''), data: {folder: folder, test: test} })
        .then(function (f) {
          var found = f.findIndex(function (v) { return v.a == a; });
    ...
    ctrl.getImageList = function () {
      return images().map(function (v, i) { return m$3('span', {class: group == i ? 'current' : '',onclick: function (e) { return group = i; }}, v.a); })
    };

    ctrl.getInfoTag = function (keys, index) {
      if (keys[index] == 'last')
        { return m$3('a[href=javascript:;]', {onmousedown:function (e){
          e.preventDefault();
          e.stopPropagation();
    ...
    ctrl.getImageTag = function () {
      ...
      var keys = Object.keys(obj);
      return [
        m$3('.info', ctrl.getInfoTag(keys, index)),
        keys.map(function (v, i) {
          return m$3('.image', {class: index !== i ? '  :global(hide)   hide  ' : ''}, m$3('img', {src: PTEST_PATH + folder + '/' + obj[v]}))
        })
      ]
    };
  ...
  view: function (ctrl, arg) {
    return m$3('.test-image-con', [
      m$3('menu.top', [
        m$3('a[href=#]', {onclick: function (e) { return arg.onclose && arg.onclose(); }}, 'close'),
        ctrl.getImageList()
      ]),
      m$3('.imageBox', {onmousedown: function (e) { return ctrl.cycleVisible(detectRightButton$1() ? -1 : 1); }}, ctrl.getImageTag())
    ])
  }
};

var testdata$1 = {'test': 'test1465218335247','folder': 'ptest_data','a': 'test1465218335247/1465218058523.png','b': 'test1465218335247/1465218058523.png_test.png','diff': 'test1465218335247/1465218058523.png_diff.png'};

//
// helper functions

function detectRightButton$1 (e) {
  var rightclick;
  if (!e) { var e = window.event; }
  if (e.which) { rightclick = (e.which == 3); }
  else if (e.button) { rightclick = (e.button == 2); }
...
var style$2 = cssobj_cjs$1({
  '.page':{}
});

var m$4 = cssobjMithril_cjs$1(style$2);

var assertPopup = {
  controller: function(arg) {
    var ctrl = this;
    ctrl.getResult = function() {
      return m$4('div', arg.data)
    };
  },
  view: function(ctrl, arg) {
    return m$4('.test-assert-con', [
      m$4('menu.top', [
        m$4('a[href=#]', {onclick: function (e) { return arg.onclose && arg.onclose(); }}, 'close'),
      ]),
      m$4('.infoBox', ctrl.getResult())
    ])
  }
};
...
var moverlay_cjs = moverlay;

var hasOwn = Object.prototype.hasOwnProperty;
var toString = Object.prototype.toString;

var index$1 = function forEach (obj, fn, ctx) {
...
var PLAYING = 'STAGE_PLAYING';
var CLIPPING = 'STAGE_CLIPPING';
var SETUP = 128;
var REPORTER = 256;
var IMAGEVIEW = 512;
var INVALID_NAME = '<>:"\\|?*'; // '<>:"/\\|?*'
...
window.stage = function (){ return stage; };
...
ws.onopen = function (e) {
  ws.onmessage = function (message) {
    ...
    case 'test_output':
      ...
      console.log(msg.data);
      stage = REPORTER;
      moverlay_cjs.show('#reporter', {com: m.component(reporter, {
        data:msg.data,
        onmsg:function(msg) {
          stage = IMAGEVIEW;
          moverlay_cjs.show('#testimage', {com: m.component(msg.type=='assert' ? assertPopup : gallary, {data: msg.error, onclose:function() {
            stage = REPORTER;
            moverlay_cjs.hide('#testimage');
          }})});
        },
        onclose:function() {
          stage = SETUP;
          moverlay_cjs.hide('#reporter');
        }
      })});
...
var oncloseSetup = function (arg) {
  ...
  if(arg.action=='view'){
    ...
    var a = arg.a;
    stage = IMAGEVIEW;
    moverlay_cjs.show( '#testimage', { com: m.component(
        gallary,
        {
          data: {folder: folder,test: test,a: a},
          onclose:function() {
            stage=SETUP;
            moverlay_cjs.hide('#testimage');
          }
...
function showSetup (arg) {
  ...
  stage = SETUP;
  moverlay_cjs.show('#overlay', {com: m.component(com, {url: '/config', onclose: oncloseSetup })});
}
